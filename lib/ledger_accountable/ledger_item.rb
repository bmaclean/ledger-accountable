...
module LedgerAccountable::LedgerItem
  ...
  included do
    ...
    around_create :record_ledger_addition, if: :should_record_ledger_addition?
    around_update :record_ledger_addition, if: :should_record_ledger_addition?
    around_update :record_ledger_removal, if: :should_record_ledger_removal?
    around_update :record_ledger_update, if: :should_record_ledger_update?
    around_destroy :record_ledger_destruction, if: :should_persist_in_ledger?
  ...
  class_methods do
    ...
    def validate_and_assign_ledger_owner(ledger_owner)
      ...
      unless instance_methods.include?(ledger_owner)
        raise "LedgerAccountable::LedgerItem model #{model_name} must respond to the provided value for ledger_owner (#{ledger_owner})."
      end
